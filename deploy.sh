...
# 4. 解压
echo "=== 服务器解压 ==="
sshpass -e ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $USER@$HOST "mkdir -p $TARGET && tar -xzf /tmp/website.tar.gz -C $TARGET && rm /tmp/website.tar.gz && echo '部署完成'"
